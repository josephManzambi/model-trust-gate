...
- **A modification** (quantization, fine-tuning, or a model merge), discovered or performed at L3 or L4, re-opens **L2** (your legal standing may shift from deployer to provider: for a modified general-purpose model under the EU AI Act GPAI provisions, or for a substantial modification of a high-risk system under Art. 25), re-opens the affected **L3/L4 evidence** under the tiered-evidence rule at Layer 3, and re-opens **L5** for any guardrail whose justifying L4 finding the modification may have changed, because the tested artifact is no longer the approved one.
...
- **What happens, for a hosted open-weight model (a third origin):** an open-lineage model served through a cloud provider you trust (for example Qwen or Llama on AWS Bedrock, Azure AI Foundry, or a similar managed endpoint) is the common middle case, and it is neither of the two above. The provider holds and runs the files, so the file-level supply chain (malware scan, format, checksum) is absorbed by them exactly as for any cloud model, and there is nothing on your disk to scan. What does **not** transfer is the safety pedigree: an open-lineage model typically arrives with far thinner inherited safety testing than a frontier lab's, so the extra scrutiny moves off L1 and onto L2 (a first-time model-origin and data-governance decision, treated below) and onto your own L3/L4 testing. At L1 itself the work is light: confirm which exact model variant and version the provider serves, and that it is pinned. Treat the origin in the record as `hosted-open-weight` so the effort lands in the right place. (Worked end to end in `runbooks/bedrock-hosted-open-weight.md`.)
...
- **Bank model-risk management (SR 11-7, superseded by SR 26-2 in April 2026)**: the structural ancestor: tier by materiality, scale validation to the tier, sign off, revalidate on a trigger. SR 26-2 explicitly excludes generative and agentic AI from its scope, which is part of why a gap exists for the Gate to fill. `[verify: SR 26-2's supersession of SR 11-7 and its stated exclusion of generative and agentic AI, against the primary supervisory letter]` The Gate is that pattern retargeted at AI-specific inputs, with the cloud-versus-open-weight weighting added.
